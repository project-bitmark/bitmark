...
int64_t GetBlockValue(CBlockIndex* pindexPrev, int64_t nFees, bool scale)
{
    ...
    if (RegTest()) return nFees + 1500000000 - 1500000000/scalingFactor*100;
...
unsigned int get_ssf (CBlockIndex * pindex) {
  unsigned int scalingFactor = 2100000000; // ensures that it has no effect
  CBlockIndex * pprev_algo = pindex;
  unsigned long hashes_peak = 0;
  unsigned long hashes_cur = 0;
  for (int i=0; i<365; i++) { // use at most a year's worth of history
    ...
    unsigned long hashes = (hashes_bn.getulong())/time_f;
    //LogPrintf("hashes per sec = %f\n",hashes);
    if (hashes>hashes_peak) hashes_peak = hashes;
    if (i==0) hashes_cur = hashes;
  }
  if (hashes_peak > 0. && hashes_cur != hashes_peak) {
    scalingFactor = (200*hashes_peak)/(hashes_peak-hashes_cur); // a 3-4 digit integer
  }
