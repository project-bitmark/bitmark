...
int64_t GetBlockValue(CBlockIndex* pindexPrev, int64_t nFees, bool scale)
{
    ...
    if (RegTest()) return nFees + 1500000000 - 1500000000/scalingFactor*100000000;
    ...
    return nFees + baseSubsidy - baseSubsidy/scalingFactor*1000000;
}
...
unsigned int get_ssf (CBlockIndex * pindex) {
  ...
  if (hashes_peak > CBigNum(0) && hashes_cur != hashes_peak) {
    scalingFactor = ((2*100000000*hashes_peak)/(hashes_peak-hashes_cur)).getuint(); // a 9-10 digit integer
  }
